...
class Model(pl.LightningModule):
    ...
    def forward(self, input, text=None, is_train=True):
        ...
        if self.stages['Pred'] == 'CTC':
            ...
            if text == None:
                ...
                text = torch.LongTensor(batch_size, self.opt.batch_max_length + 1).fill_(0)
    ...
    def configure_optimizers(self):
        ...
        # setup optimizer
        if self.opt.adam:
            return optim.Adam(filtered_parameters, lr=self.opt.lr, betas=(self.opt.beta1, 0.999))
        else:
            return optim.Adadelta(filtered_parameters, lr=self.opt.lr, rho=self.opt.rho, eps=self.opt.eps)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict({'val-loss': cost, 'val-acc': accuracy, 'val-ned': norm_ED}, 
                      prog_bar=True, batch_size=self.opt.batch_size)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log_dict({'test-loss': cost, 'test-accuracy': accuracy, 'test-norm_ED': norm_ED},
                      prog_bar=True, batch_size=self.opt.batch_size)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for img_name, pred, pred_max_prob in zip(image_path_list, preds_str, preds_max_prob):
            ...
            # calculate confidence score (= multiply of pred_max_prob)
            confidence_score = pred_max_prob.cumprod(dim=0)[-1] if len(pred_max_prob.cumprod(dim=0)) > 0 else 0.
